...
== Restriction on PRs for DSL and validator

If you are proposing a change to the Marlowe domain-specific language (DSL) or to the Plutus validator for Marlowe, pursue the https://github.com/input-output-hk/MIPs[Marlowe Improvement Proposal (MIP)] process by starting a https://github.com/input-output-hk/MIPs/discussions[MIP discussion] of the proposed change. *Pull requests for DSL or validator changes will be rejected unless they have previously been approved via the MIP process.*
...
NOTE: You may want to consider using direnv as a convenient alternative to running `nix develop` directly.
...
. Check that you can run `nix build .#cabalProject.<SYSTEM>.hsPkgs.<PACKAGE>.exec.<COMPONENT>` successfully.
...
* If it is an `source-repository-package`/`extra-dep` from Git, you update the `sha256` mapping in `nix/project.nix`.
...
We use `fourmolu` for Haskell code formatting, `cabal-fmt` for cabal, and `nixpkgs-fmt` for Nix.
CI checks that running these is a no-op, so if you don’t apply it then your PR will not go green.
To avoid annoyance, set up your editor to run them automatically.
The `nix develop` environment provides all the necessary binaries of the correct versions.

You can run all the formatters over the repo using `pre-commit run --all`.
...
A checklist for changes that require review is provided in the PR template.
...
All changes to the product require changelog entries. We use Scriv to manage changelog fragments and then assemble them when cutting a release.
For more information please see https://scriv.readthedocs.io/en/1.1.0/index.html

The components that require changelogs are:

- marlowe-runtime (incl chain-sync, cli, web, client). Use `changelog.d` in `marlowe-runtime`
- marlowe-cardano. Use `changelog.d` in `marlowe`
- marlowe-actus. Use `changelog.d` in `marlowe-actus`
- marlowe-cli. Use `changelog.d` in `marlowe-cli`

CI will check that you have updated the changelog in a PR. If your PR doesn't require a changelog entry, add the "No Changelog Required" label to it.
