...
VIf you launch `nix-shell` with the `enableHaskellProfiling` argument set to true, you will get a shell where all the dependencies have been built with profiling.
...
. Check that you can run `nix build -f default.nix marlowe.haskell.projectPackages.<package name>` successfully.
...
The `nix-shell` environment provides `stylish-haskell`, `purs-tidy`, and `prettier` binaries of the correct versions.

You can run `stylish-haskell`, `purs-tidy`, or `prettier` over your tree using the `fix-stylish-haskell`, `fix-purs-tidy`, or `fix-prettier` scripts provided by the `nix-shell` environment.
