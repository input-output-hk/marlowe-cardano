  ...
  withEvent eventBackend (DynamicEventSelector "Contract") $
    \event ->
      do
        ...
        let subBackend = hoistEventBackend liftIO $ subEventBackend idInjectSelector event eventBackend
        result <-
          runExceptT $
            do
              now <- currentTime
  ...
  do
    ...
    let commandOptions =
          Command
            ...
            <*> (O.many . O.argument addressKeyfileParser)
              (O.metavar "ADDRESS=KEYFILE" <> O.help "The addresses of the parties and the files with their signing keys.")
    pure $
      O.info
