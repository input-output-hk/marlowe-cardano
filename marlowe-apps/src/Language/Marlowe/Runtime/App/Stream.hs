...
streamContractHeadersClient eventBackend (PollingFrequency pollingFrequency) endOnWait extract channel =
  let clientIdle = HSync.SendMsgRequestNext clientNext
      clientWait
              ...
              liftIO . withEvent eventBackend (DynamicEventSelector "HeadersClientNew") $
                \event ->
                  do
                    ...
                    let extracted = mapMaybe (extract . Right) results
              ...
              liftIO . withEvent eventBackend (DynamicEventSelector "HeadersClientRollback") $
                \event ->
                  do
                    ...
                    let extracted = extract $ Left chainPoint
                    atomically $ mapM_ (writeTChan channel . Right) extracted
                    pure clientIdle
          , HSync.recvMsgWait =
              liftIO
   ...
   in HSync.MarloweHeaderSyncClient $
        pure clientIdle
                  ...
                  \event ->
                    do
                      ...
                      case version `assertVersionsEqual` (marloweVersion :: MarloweVersion v) of
                        Refl -> do
                          let accepted = accept $ Left csCreateStep
              ...
              liftIO . withEvent eventBackend (DynamicEventSelector "StepsClientApplyForward") $
                \event ->
                  do
                    ...
                    let acceptances = accept . Right <$> steps
                    ...
                    if and acceptances
                      then
                        if unFinishOnClose finishOnClose && any hasClosed steps
                          then do
                            atomically
                              . writeTChan channel
                              . Right
