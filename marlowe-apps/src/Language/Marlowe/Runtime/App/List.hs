...
listContracts cFilter =
  let append = (. Query.getContractHeaders cFilter) . (=<<) . handleNextPage
      handleNextPage previous Nothing = pure previous
      handleNextPage previous (Just Query.Page{..}) =
        let cumulative = previous <> items
         in case nextRange of
              Nothing -> pure cumulative
              Just range -> cumulative `append` range
   in runMarloweQueryClient $ mempty `append` Query.Range Nothing 0 pageSize Query.Ascending
...
getContract contractId' =
  let stNext version create previous =
        Sync.ClientStNext
          ...
          , Sync.recvMsgRollForward = \_ steps ->
              pure
   ...
   in runMarloweSyncClient
        . Sync.MarloweSyncClient
        . pure
        $ Sync.SendMsgFollowContract
          contractId'
          Sync.ClientStFollow
