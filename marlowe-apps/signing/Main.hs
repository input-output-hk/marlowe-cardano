    ...
    withObject "SignRequest"
      $ \o ->
        do
          ...
          reqPaymentExtendedKeys <- mapM (textEnvelopeFromJSON $ C.AsSigningKey C.AsPaymentExtendedKey) =<< o .: "paymentExtendedKeys"
          pure Sign{..}
...
signingHandler Sign{..} =
  let
    (resTxId, resTx) = sign reqTxBody reqPaymentKeys reqPaymentExtendedKeys
  in
    pure Tx{..}
