...
simpleHandshakeServer expected server =
  HandshakeServer
...
hoistHandshakeServer hoistServer f HandshakeServer{..} =
  HandshakeServer
...
handshakeServerPeer serverPeer HandshakeServer{..} =
  AwaitTraced (ClientAgency TokInit) \case
    MsgHandshake sig ->
      Respond (ServerAgency TokHandshake) $
  ...
  where
    ...
    liftPeer = \case
      ...
      AwaitTraced (ClientAgency tok) k -> AwaitTraced (ClientAgency $ TokLiftClient tok) \(MsgLift msg) -> case k msg of
        Respond (ServerAgency tok') next ->
          Respond (ServerAgency $ TokLiftServer tok') $
            next <&> \case
