...
genMsgRejectHandshake :: Gen (Message (Job TestCommand) 'StHandshake 'StFault)
...
spec = do
  ...
  it "MsgConfirmHandshake serialization roundtrip" $ do
    let
      msg :: Message (Job TestCommand) 'StHandshake 'StIdle
      msg = MsgConfirmHandshake
    prop_codec runIdentity codecJob (AnyMessageAndAgency (ServerAgency TokHandshake) msg) `shouldBe` True
  prop "MsgRejectHandshake serialization roundtrip" do
    msg <- genMsgRejectHandshake
    prop_codecM codecJob (AnyMessageAndAgency (ServerAgency TokHandshake) msg)
  it "MsgDone serialization roundtrip" $ do
    let
      msg :: Message (Job TestCommand) 'StIdle 'StDone
      msg = MsgDone
  ...
  it "MsgExec serialization roundtrip" $ do
    let
      msg :: Message (Job TestCommand) 'StIdle ('StCmd () () ())
