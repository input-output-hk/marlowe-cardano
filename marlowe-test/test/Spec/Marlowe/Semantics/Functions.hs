  ...
  testGroup
    ...
    , testGroup
        ...
        , testGroup
            ...
            , testProperty "Amount does not match" $
                checkIDeposit Nothing Nothing Nothing (Just False)
            , testProperty "AccountId, party, token, and amount match" $
                checkIDeposit (Just True) (Just True) (Just True) (Just True)
            ]
        , testGroup
            "IChoice"
...
checkFixInterval
  :: Bool
  -- ^ Whether the validity interval should be invalid.
  -> Bool
  -- ^ Whether the validity interval should be in the past.
  -> Property
  -- ^ The test.
...
checkAvailableMoney
  :: Bool
  -- ^ Whether the account is present in the state.
  -> Property
  -- ^ The test.
checkAvailableMoney isElement =
  let gen _ State{accounts} =
        if isElement && not (AM.null accounts)
          then elements $ AM.keys accounts
          else arbitrary
   in checkValue gen $ \eval _ _ State{accounts} (account, token) ->
        let x = AvailableMoney account token
         in case (account, token) `AM.lookup` accounts of
              Nothing -> eval x == 0
              Just x' -> eval x == x'
...
checkChoiceValue
  :: Bool
  -- ^ Whether the choice should be taken from the state.
  -> Property
  -- ^ The test.
checkChoiceValue isElement =
  let gen _ State{choices} =
        if isElement && not (AM.null choices)
          then elements $ AM.keys choices
          else arbitrary
   in checkValue gen $ \eval _ _ State{choices} choice ->
        let x = ChoiceValue choice
         in case choice `AM.lookup` choices of
              Nothing -> eval x == 0
              Just x' -> eval x == x'
...
checkUseValue
  :: Bool
  -- ^ Whether the bound value is present in the state.
  -> Property
  -- ^ The test.
checkUseValue isElement =
  let gen _ State{boundValues} =
        if isElement && not (AM.null boundValues)
          then elements $ AM.keys boundValues
          else arbitrary
   in checkValue gen $ \eval _ _ State{boundValues} variable ->
        let x = UseValue variable
         in case variable `AM.lookup` boundValues of
              Nothing -> eval x == 0
              Just x' -> eval x == x'
...
checkChoseSomething
  :: Bool
  -- ^ Whether the choice is present in the state.
  -> Property
  -- ^ The test.
checkChoseSomething isElement =
  let gen _ State{choices} =
   ...
   in checkValue gen $ \_ eval _ State{choices} choice ->
...
checkIDeposit accountMatches partyMatches tokenMatches amountMatches = property $ do
  ...
  let gen = do
        ...
        pure
          ...
          , token'
          , amount'
          , Deposit account party token amount
          , accountMatches' && partyMatches' && tokenMatches' && amountMatches'
          )
  forAll' gen $ \(environment, state, account, party, token, amount, action, match) ->
    let amount' = maybe amount (+ amount) . AM.lookup (account, token) $ accounts state
        newState = state{accounts = AM.insert (account, token) amount' $ accounts state}
     in case applyAction environment state (IDeposit account party token amount) action of
          NotAppliedAction -> not match
          AppliedAction ApplyNoWarning state' -> match && amount > 0 && newState == state'
          AppliedAction (ApplyNonPositiveDeposit party' account' token' amount'') state' ->
            match
              && amount <= 0
              && state == state'
              && party == party'
...
checkIChoice
  ...
  -> Maybe Bool
  -- ^ Whether the choice is in bounds.
  -> Property
  -- ^ The test.
checkIChoice choiceMatches choiceInBounds = property $ do
  ...
  forAll' gen $ \(environment, state, choiceId, choiceNum, action, match) ->
    let newState = state{choices = AM.insert choiceId choiceNum $ choices state}
     in case applyAction environment state (IChoice choiceId choiceNum) action of
          NotAppliedAction -> not match
...
checkINotify = property $ do
  let gen = do
        ...
        pure (environment, state, x)
  forAll' gen $ \(environment, state, x) ->
    let result = evalObservation environment state x
     in case applyAction environment state INotify (Notify x) of
  ...
  property $ do
    let gen =
          do
            ...
            (,,) <$> semiArbitrary context <*> semiArbitrary context <*> semiArbitrary context
    forAll' gen $ \((account, token), amount, accounts') ->
      let newAccounts = assocMapAdd (account, token) amount accounts'
  ...
  property $ do
    ...
    forAll' gen $ \((account, token), amount, payee, accounts') ->
      let newAccounts =
       ...
       in (if amount > 0 then newAccounts else accounts') `assocMapEq` accounts''
            && case result of
              ReduceWithPayment (Payment account'' payee'' token' amount') ->
                if amount' /= 0
                  then
                    account'' == account
                      && payee == payee''
...
checkReduceContractStepClose =
  property $ do
    forAll' arbitrary $ \(environment, state) ->
      let checkPayment (Payment payee (Party payee') token amount) state' Close =
            payee == payee'
              && amount /= 0
       ...
       in case reduceContractStep environment state Close of
  ...
  property $ do
    ...
    forAll' gen $ \(environment, state, account, payee, token, value, contract) ->
      let prior = fromMaybe 0 $ AM.lookup (account, token) (accounts state)
          ...
          newState =
            state
              { accounts = (if posterior == 0 then AM.delete else flip assocMapInsert posterior) (account, token) (accounts state)
              }
          ...
          checkPayment (Payment account' (Account payee') token' amount) state' =
            let other = fromMaybe 0 $ AM.lookup (payee', token) (accounts newState)
                newState' =
                  if other + debit > 0
                    then newState{accounts = assocMapInsert (payee', token) (other + debit) (accounts newState)}
                    else newState
             in account' == account
                  && Account payee' == payee
                  && if amount /= 0
       ...
       in case reduceContractStep environment state (Pay account payee token value contract) of
            Reduced (ReduceNonPositivePay account' payee' token' request') ReduceNoPayment state' contract' ->
              not positiveAmount
                && account' == account
                && payee' == payee
                && token' == token
                ...
                && state' `stateEq` state
                && contract' == contract
            Reduced ReduceNoWarning (ReduceWithPayment payment) state' contract' ->
              positiveAmount
                && fullAmount
                && checkPayment payment state'
                && contract' == contract
            Reduced (ReducePartialPay account' payee' token' debit' request') (ReduceWithPayment payment) state' contract' ->
              (positiveAmount || debit' == 0)
                && not fullAmount
                && account' == account
                && payee' == payee
...
checkReduceContractStepIf =
  property $ do
    forAll' arbitrary $ \(environment, state, observation, thenContract, elseContract) ->
      let passed = evalObservation environment state observation
       in case reduceContractStep environment state (If observation thenContract elseContract) of
...
checkReduceContractStepWhen =
  property $ do
    forAll' arbitrary $ \(environment, state, cases, timeout, contract) ->
      let before = snd (timeInterval environment) < timeout
          afterwards = fst (timeInterval environment) >= timeout
       in case reduceContractStep environment state (When cases timeout contract) of
            NotReduced -> before
...
checkReduceContractStepLet =
  property $ do
    forAll' arbitrary $ \(environment, state, valueId, value, contract) ->
      let x = evalValue environment state value
          shadow = valueId `AM.member` boundValues state
       in case (shadow, reduceContractStep environment state (Let valueId value contract)) of
            (False, Reduced ReduceNoWarning ReduceNoPayment state' contract') ->
              contract == contract'
                && state{boundValues = assocMapInsert valueId x (boundValues state)} `stateEq` state'
            (True, Reduced (ReduceShadowing valueId' value' x') ReduceNoPayment state' contract') ->
              contract == contract'
                && state{boundValues = assocMapInsert valueId x (boundValues state)} `stateEq` state'
                && valueId == valueId'
...
checkReduceContractStepAssert =
  property $ do
    forAll' arbitrary $ \(environment, state, observation, contract) ->
      let passed = evalObservation environment state observation
       in case reduceContractStep environment state (Assert observation contract) of
