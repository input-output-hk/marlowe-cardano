  ...
  let randomContext =
      ...
      goldenContext =
        do
          ...
          pure MarloweContext{..}
   in frequency [(1, randomContext), (2, goldenContext)]
...
simplePayment =
  do
    ...
    let modifyContract _ contract = pure $ Pay account (Party payee) token (Constant payment) contract
        modifyState state =
          do
            accounts' <-
              fmap AM.fromList
...
simpleDeposit =
  do
    ...
    payee <- semiArbitrary context
    token <- semiArbitrary context
    let modifyContract (_, POSIXTime intervalEnd) contract =
          pure $
            When
              [Case (Deposit account payee token $ Constant deposit) contract]
              (POSIXTime $ intervalEnd + 1)
              contract
...
simpleChoice =
  do
    ...
    let modifyContract (_, POSIXTime intervalEnd) contract =
          do
            let timeout = POSIXTime $ intervalEnd + 1
...
data Invariant
  = -- | The pre- and post-transaction states are identical.
    SameState
  | -- | The pre- and post-transaction account records are identical.
    SameAccounts
  | -- | The pre- and post-transaction choice records are identical.
    SameChoices
  | -- | The pre- and post-transaction bound values are identical.
    SameValues
  | -- | The pre- and post-transaction minimum times in the states are identical.
    SameTime
  | -- | The pre- and post-transaction contracts are identical.
    SameContract
...
requirePayout =
  let isPayout (Payment _ (Party _) _ i) = i > 0
      isPayout _ = False
   in view payments >>= "Positive payout" `require` any isPayout

-- | Require not deposits.
requireNoDeposits :: Testify ()
requireNoDeposits =
  let isDeposit (NormalInput (IDeposit _ _ _ i)) = i > 0
      isDeposit (MerkleizedInput (IDeposit _ _ _ i) _ _) = i > 0
      isDeposit _ = False
   in view inputs >>= "No deposits" `require` (not . any isDeposit)
...
makesPayments ps =
  do
    ps' <- view payments
    let okay = ((==) `on` (sort . fmap unPayment)) ps ps'
    unless okay $
...
data TransactionTest = TransactionTest
  { name :: String
...
test TransactionTest{..} =
  testProperty name
    . property
    $ let preResolve :: Testify () -> MarloweContext -> Bool
       ...
       in (if allowShrinkage then forAllShrink gen shrink else forAll gen)
            . postResolve
