...
toJsonable
  ...
  -> Value
  -- ^ The JSON value.
  -> Either String Jsonable
  -- ^ The deserialized value, or an error message.
toJsonable known s v =
  let fromJson' :: forall a. (FromJSON a) => Proxy a -> Either String a
      fromJson' _ =
        case fromJSON v :: Result a of
          Success x -> Right x
          Error msg -> Left msg
   in case filter ((s ==) . typeKey) known of
...
isKnownJson
  ...
  -- ^ The key for the type.
  -> Bool
  -- ^ Whether the type is in the list of known JSON types.
...
roundTripJsonable
  ...
  -- ^ The JSON value.
  -> Either String Value
  -- ^ There the re-serialized value, or an error message.
...
checkRoundTripJsonable
  ...
  -- ^ The JSON value.
  -> Bool
  -- ^ Whether the JSON value is unchanged.
...
arbitraryJsonable JsonableType{..} =
  let gen :: (Arbitrary a) => Proxy a -> Gen a
      gen _ = arbitrary
   in Jsonable typeKey <$> gen typeValue
