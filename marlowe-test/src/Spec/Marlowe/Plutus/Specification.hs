    ...
    [
      testGroup "Semantics Validator"
        ...
        , testGroup "Constraint 20. Single satisfaction"
            [
              testProperty "Invalid other validators during payment" checkOtherValidators
            ]
...
checkOtherValidators :: Property
checkOtherValidators =
  let
    modifyAfter =
      -- Add an extra script input.
      infoInputs <><~ lift (listOf1 $ arbitrary `suchThat` isScriptTxIn)
  in
    checkSemanticsTransaction noModify modifyAfter hasPayouts False False
