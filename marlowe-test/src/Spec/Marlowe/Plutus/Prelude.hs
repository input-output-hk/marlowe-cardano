...
checkFoldMap =
  property
    . forAll (arbitrary :: Gen [Double])
    $ \xs ->
      let f x = [i * x + 2 | i <- [0 .. 10]]
       in P.foldMap f xs == foldMap f xs
...
checkEqDatumHash =
  property
    . forAll arbitrary
    $ \(x, y) ->
      let x' = DatumHash $ stringToBuiltinByteString x
...
checkEqTxOutRef =
  property
    . forAll arbitrary
    $ \(x, i, y, j) ->
      let xi = TxOutRef (TxId $ stringToBuiltinByteString x) i
