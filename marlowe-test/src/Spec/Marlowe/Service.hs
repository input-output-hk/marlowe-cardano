...
handle ComputeTransaction{..} =
  let valueResponse = A.toJSON $ Marlowe.computeTransaction transactionInput state contract
   in pure RequestResponse{..}
handle PlayTrace{..} =
  let valueResponse = A.toJSON $ Marlowe.playTrace initialTime contract transactionInputs
   in pure RequestResponse{..}
handle EvalValue{..} =
  let valueResponse = A.toJSON $ Marlowe.evalValue environment state value
   in pure RequestResponse{..}
handle EvalObservation{..} =
  let valueResponse = A.toJSON $ Marlowe.evalObservation environment state observation
   in pure RequestResponse{..}
