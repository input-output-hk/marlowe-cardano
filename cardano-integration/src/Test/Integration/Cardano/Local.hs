...
instance Exception TestnetException
...
startLocalTestnet options@LocalTestnetOptions{..} = do
  ...
  catchAny
    do
      ...
      (byronGenesisDir, shelleyGenesisDir) <-
        concurrently
      ...
      spoNodes <-
        traverse (setupSpoNode workspace options network logsDir socketDir byronGenesisDir shelleyGenesisDir) [1 .. numSpoNodes]
...
createByronGenesis workspace startTime testnetMagic LocalTestnetOptions{..} = do
  byronGenesisSpecFile <-
    writeWorkspaceFileJSON workspace "byron.genesis.spec.json" $
      object
  ...
  execCli_
    ...
    , show testnetMagic
    , "--start-time"
    , show @Int $ floor $ nominalDiffTimeToSeconds $ utcTimeToPOSIXSeconds startTime
    , "--k"
    , show securityParameter
    , "--n-poor-addresses"
    , "0"
...
setupSpoNode workspace LocalTestnetOptions{..} Network{..} logsDir socketDir byronGenesisDir shelleyGenesisDir n = do
  ...
  let movePoolFile name extension =
        moveToWorkspace
          workspace
          (shelleyGenesisDir </> "pools" </> name <> show n <> "." <> extension)
          (nodeName </> name <> "." <> extension)
  ...
  byronDelegateKey <-
    moveToWorkspace
      workspace
      (byronGenesisDir </> "delegate-keys." <> printf "%03d" (n - 1) <> ".key")
      (nodeName </> "byron-delegate.key")
  byronDelegationCert <-
    moveToWorkspace
      workspace
      (byronGenesisDir </> "delegation-cert." <> printf "%03d" (n - 1) <> ".json")
      (nodeName </> "byron-delegation.cert")
  ...
  topology <-
    writeWorkspaceFileJSON workspace (nodeName </> "topology.json") $
      object
