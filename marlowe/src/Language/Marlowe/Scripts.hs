...
{-
 ...
 -}
mkRolesCurrency :: MarloweParams -> CurrencySymbol
...
smallMarloweValidator MarloweParams{rolePayoutValidatorHash}
    ...
    let findOwnInput :: ScriptContext -> Maybe TxInInfo
        ...
        (ownAddress, ownValidatorHash) =
          case ownInput of
            TxInInfo{txInInfoResolved=TxOut{txOutAddress=ownAddress'@(Address (ScriptCredential (ValidatorHash ownValidatorHash')) _)}} -> (ownAddress', ownValidatorHash')
            _ -> traceError "I2"  -- FIXME: Review this pattern match, per PR-117.
        ...
        marloweTxInputToInput (MerkleizedTxInput input hash) =
            case findDatum (DatumHash hash) scriptContextTxInfo of
                Just (Datum d) -> let
                    ...
                    continuation = fromDataToContract d   -- FIXME: Check this!
