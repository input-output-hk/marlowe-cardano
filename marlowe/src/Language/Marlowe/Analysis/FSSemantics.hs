...
convertToSymbolicTrace refL symL =
  let lenRefL = length refL
      lenSymL = length symL
   in if lenRefL <= lenSymL
        then
          let lenPadding = lenSymL - lenRefL
           in isPadding (take lenPadding symL) .&& convertRestToSymbolicTrace refL (drop lenPadding symL)
...
isValidAndFailsWhen
  ...
  pos =
    do
      ...
      return
        ( ite
            (newCond .&& sNot clashResult .&& ensureBounds concVal bnds)
            newTrace
            contTrace
        )
isValidAndFailsWhen
...
isValidAndFailsWhen
  oa
  hasErr
  (MerkleizedCase (Deposit accId party token val) _ : rest)
  timeout
  timCont
  previousMatch
  sState
...
isValidAndFailsWhen
  oa
  hasErr
  (MerkleizedCase (Choice choId bnds) _ : rest)
  timeout
  timCont
  previousMatch
  sState
  pos =
    let newPreviousMatch otherSymInput pmSymState =
          case otherSymInput of
            ...
            _ -> previousMatch otherSymInput pmSymState
     in isValidAndFailsWhen oa hasErr rest timeout timCont newPreviousMatch sState (pos + 1)
isValidAndFailsWhen
  oa
  hasErr
  (MerkleizedCase (Notify obs) _ : rest)
  timeout
  timCont
  previousMatch
  sState
