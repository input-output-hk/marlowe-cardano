...
marloweFollowContract = awaitPromise $ endpoint @"follow" $ \params ->
  do
    ...
    let
      ...
      -- of the contract context when contract is missing from the chain.
      ...
      -- Essentially follower is a `do {..} while` loop:
      --  - we use `QueryResult` wrapper so *every* chain query is wrapped in `checkpointLoop`.
      --  - we use a bit better heuristic when waiting for the chain index then
      --    arbitrary timeout. We wait till the result of the query differs from
  ...
  where
    ...
    lastUtxo = foldlHistory step Nothing
      where
        next Created { historyNext=n }      = n
        next InputApplied { historyNext=n } = n
        next Closed {}                      = Nothing

        step _ entry = case (next entry, entry) of
          (Nothing, Created { historyTxOutRef })      -> Just historyTxOutRef
          (Nothing, InputApplied { historyTxOutRef }) -> Just historyTxOutRef
          (_, _)                                      -> Nothing

    isClosed = foldlHistory step False
      where
        step _ Closed {} = True
        step _ _         = False
