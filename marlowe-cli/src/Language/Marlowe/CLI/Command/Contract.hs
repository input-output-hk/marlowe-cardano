...
import           Cardano.Api                     (AddressAny, ConsensusModeParams (CardanoModeParams), EpochSlots (..),
                                                  LocalNodeConnectInfo (..), NetworkId (..), SlotNo,
                                                  StakeAddressReference (..), TxIn)
import           Control.Monad                   (when)
import           Control.Monad.Except            (MonadError, MonadIO, liftIO, throwError)
import           Data.Maybe                      (fromMaybe)
import           Language.Marlowe.CLI.Parse      (parseAddressAny, parseCurrencySymbol, parseInput, parseNetworkId,
                                                  parseSlotNo, parseStakeAddressReference, parseTxIn, parseTxOut)
...
initializeCommand :: O.Mod O.CommandFields ContractCommand
initializeCommand =
...
initializeOptions :: O.Parser ContractCommand
initializeOptions =
  Initialize
...
prepareCommand :: O.Mod O.CommandFields ContractCommand
prepareCommand =
  O.command "prepare"
...
prepareOptions :: O.Parser ContractCommand
prepareOptions =
  Prepare
...
runCommand :: O.Mod O.CommandFields ContractCommand
runCommand =
  O.command "execute"
    $ O.info runOptions
...
runOptions :: O.Parser ContractCommand
runOptions =
  Run
    <$> (O.optional . O.option parseNetworkId) (O.long "testnet-magic"       <> O.metavar "INTEGER"       <> O.help "Network magic, or omit for mainnet."                                )
