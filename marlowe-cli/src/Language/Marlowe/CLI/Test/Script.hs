...
import Language.Marlowe.CLI.Types (CliEnv (..), CliError (..), MarloweTransaction (mtState),
...
scriptTest era costModel connection faucet slotConfig ScriptTest{..} =
  do
    liftIO $ putStrLn ""
    ...
    void $ catchError
      (runReaderT (execStateT interpretLoop (scriptState faucet)) (ScriptEnv connection costModel era slotConfig))
      $ \e -> do
...
buildParty :: (MonadState (ScriptState era) m, MonadError CliError m) => Maybe CurrencyNickname -> PartyRef -> m Party
...
rewritePartyRefs :: (MonadIO m, MonadState (ScriptState era) m, MonadError CliError m) => A.Value -> m A.Value
...
decodeContractJSON :: MonadIO m => MonadState (ScriptState era) m => MonadError CliError m => A.Value -> m M.Contract
...
decodeInputJSON :: MonadIO m => MonadState (ScriptState era) m => MonadError CliError m => A.Value -> m M.Input
...
createCollaterals :: MonadReader (ScriptEnv era) m => MonadIO m => MonadState (ScriptState era) m => MonadError CliError m => Int -> Either WalletNickname (Wallet era) -> m (NonEmpty (AUTxO era), [AUTxO era])
...
ensureWalletCollaterals :: MonadIO m => MonadReader (ScriptEnv era) m => MonadState (ScriptState era) m => MonadError CliError m => Either WalletNickname (Wallet era) -> m (NonEmpty (AUTxO era), [AUTxO era])
