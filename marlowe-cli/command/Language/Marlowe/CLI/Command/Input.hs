...
data InputCommand
  = -- | Input a deposit to a contract.
    InputDeposit
      ...
      , token :: Maybe Token
      -- ^ The token being deposited, if not Ada.
      , amount :: Integer
...
runInputCommand InputDeposit{..} =
  makeDeposit
    ...
    amount
    outputFile
runInputCommand InputChoice{..} =
  makeChoice
    choiceName
    choiceParty
    chosen
    outputFile
runInputCommand InputNotify{..} =
  makeNotification
    outputFile
...
inputDepositOptions =
  InputDeposit
    <$> O.option
      parseParty
      (O.long "deposit-account" <> O.metavar "PARTY" <> O.help "The account for the deposit.")
    <*> O.option
      parseParty
      (O.long "deposit-party" <> O.metavar "PARTY" <> O.help "The party making the deposit.")
    <*> (O.optional . O.option parseToken)
      (O.long "deposit-token" <> O.metavar "TOKEN" <> O.help "The token being deposited, if not Ada.")
    <*> O.option
      O.auto
      (O.long "deposit-amount" <> O.metavar "INTEGER" <> O.help "The amount of token being deposited.")
    <*> (O.optional . O.strOption)
      (O.long "out-file" <> O.metavar "OUTPUT_FILE" <> O.help "JSON output file for contract input.")
...
inputChoiceOptions =
  InputChoice
    <$> O.strOption
      (O.long "choice-name" <> O.metavar "NAME" <> O.help "The name of the choice made.")
    <*> O.option
      parseParty
      (O.long "choice-party" <> O.metavar "PARTY" <> O.help "The party making the choice.")
    <*> O.option
      O.auto
      (O.long "choice-number" <> O.metavar "INTEGER" <> O.help "The number chosen.")
    <*> (O.optional . O.strOption)
      (O.long "out-file" <> O.metavar "OUTPUT_FILE" <> O.help "JSON output file for contract input.")
