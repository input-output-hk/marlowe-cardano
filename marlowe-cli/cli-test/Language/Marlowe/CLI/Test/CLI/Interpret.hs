...
autoRunTransaction currency defaultSubmitter prev curr@T.MarloweTransaction {..} invalid = do
  let
    log' = logStoreLabeledMsg ("autoRunTransaction" :: String)
  ...
  (submitterNickname, Wallet address _ skey _) <- foldM getInputParty Nothing mtInputs >>= \case
  ...
  case filter isMarloweOut meOuts of
    ...
    []                        -> pure (txBody, Nothing)
    _                         -> throwError $ testExecutionFailed' "[AutoRun] Multiple Marlowe outputs detected - unable to handle them yet."
  ...
  case executionMode of
    OnChainMode {} -> do
      ...
      let
        plan = do
          let
            whole = reverse $ List.NonEmpty.toList _ciPlan
...
interpret co@Withdraw {..} =
  ...
  let
    roles = P.flattenValue onChainValue `foldMapFlipped` \(cs, tn, _) ->
      if cs == ccCurrencySymbol
        then [tn]
        else mempty
  when (roles == mempty) $
    throwLabeledError co $ testExecutionFailed' $ fold
    [ "Provided wallet "
  ...
  txBodies <- foldMapMFlipped roles \role -> do
    let
      lastWithdrawalCheckPoint = Map.lookup role _ciWithdrawalsCheckPoints
    ...
    if anyWithdrawalsExist
      then do
        ...
        logStoreLabeledMsg co $ "Withdrawing funds for role " <> show role <> " after application of inputs: " <> do
          let
