...
toTestWallet ::
  ...
  => (MonadIO m) =>
       (MonadReader (Env lang era resource) m) =>
         TestRunnerFaucet era -> FaucetToWalletConversion -> m (Wallet era)
toTestWallet faucet@TestRunnerFaucet {..} conversion = do
  let expectedBalance =
        _trInitialBalance <>
        P.inv
          (_trSubmittedTransactions `foldMapFlipped`
           (lovelaceToPlutusValue . overSomeTxBody' txBodyFee)) <>
        P.inv
          (_trFundedTransactions `foldMapFlipped`
           (lovelaceToPlutusValue . overSomeTxBody' txBodyFee))
  ...
  when (actualBalance /= expectedBalance) $ do
    ...
    liftIO $
      hPutStrLn stderr $ Text.unpack (A.renderValue (A.toJSON actualBalance))
  pure $
    case conversion of
      ExcludeAllTransactions ->
        Wallet
...
updateFaucet TestRunnerFaucet {..} InterpretState {_isWallets = wallets} =
  TestRunnerFaucet
    ...
    , _trSubmittedTransactions =
        _trSubmittedTransactions <>
        view waSubmittedTransactions (getFaucet wallets)
    , _trFundedTransactions =
        _trFundedTransactions <>
        concatMap (view waSubmittedTransactions) (getNonFaucetWallets wallets)
    }
...
fetchAddressUTxOs ::
     forall era lang m resource. (MonadIO m)
  => (MonadReader (Env lang era resource) m) =>
       AddressInEra era -> m (C.UTxO era)
fetchAddressUTxOs address = do
  connection <- view envConnection
  runCli $
    queryInEra connection .
    C.QueryUTxO . C.QueryUTxOByAddress . S.singleton . T.toAddressAny' $
    address

fetchAddressTotalValue ::
     (MonadIO m)
  => (MonadReader (Env lang era resource) m) =>
       AddressInEra era -> m CS.Value
...
execWalletOperations ::
     forall era lang m resource. (IsShelleyBasedEra era)
  => (IsPlutusScriptLanguage lang) =>
       (MonadIO m) =>
         (MonadReader (Env lang era resource) m) =>
           InterpretState lang era -> [Wallet.WalletOperation] -> m (Either ( TestRunnerError
                                                                            , InterpretState lang era) (InterpretState lang era))
execWalletOperations interpretState operations =
  runExceptT
    do (testEnv, _) <- lift setupTestInterpretEnv
       let operations' = operations <&> WalletOperation
       stateRef <- liftIO $ newIORef interpretState
       let loop =
             unsafeExecIOStateT stateRef $
             runExceptT $ flip runReaderT testEnv $ for operations' interpret
       result <- liftIO loop
...
setupTestInterpretEnv = do
  ...
  runtimeSetup <-
    for
      ...
      \RuntimeConfig {..} -> do
        ...
        liftIO $
          putStrLn $
          "Connecting to Marlowe runtime at " <>
          rcRuntimeHost <> ":" <> show rcRuntimePort
        liftIO $ putStrLn $ "The whole config is: " <> show config
  ...
  let protocolVersion =
      ...
      env =
        InterpretEnv
          ...
          , _ieRuntimeMonitor =
              do (_, (runtimeMonitorContracts, runtimeMonitorInput, _)) <-
                   runtimeSetup
  ...
  pure $
    (env, )
      do (_, (_, _, runtimeMonitor)) <- runtimeSetup
         pure runtimeMonitor
...
interpretTest ::
     forall lang era. (IsShelleyBasedEra era)
  => (IsPlutusScriptLanguage lang) =>
       [TestOperation] -> PrevResult (TestResult lang era) (TestResult lang era) -> TestInterpreterM lang era (Either (TestResult lang era) (TestResult lang era))
    ...
    case possibleRuntimeMonitor of
      Just runtimeMonitor -> do
        let runRuntimeMonitor =
              liftIO
                do Runtime.runMonitor runtimeMonitor `catch` \(err :: SomeException) -> do
...
data TestRunnerEnvResource lang era =
...
acquireTestInterpretContext = do
  ...
  stateRef <-
    do faucetWallet <- toTestWallet faucet ExcludeAllTransactions
...
releaseTestInterpretContext ::
  ...
  => (IsShelleyBasedEra era) =>
       (IsPlutusScriptLanguage lang) =>
         (MonadReader (TestRunnerEnv lang era) m) =>
           TestInterpretContext lang era -> m ()
...
runTest ::
     forall era lang. (IsPlutusScriptLanguage lang)
  => (IsShelleyBasedEra era) =>
       (FilePath, TestCase) -> TestRunnerM lang era ()
runTest (testFile, TestCase {testName, operations = testOperations}) = do
  ...
  whenM
    (view envStreamJSON)
    do resultJson <- testResultToJSON testFile testName result
...
acquireFaucets ::
     forall era lang m. (IsShelleyBasedEra era)
  => (MonadIO m) =>
       (IsPlutusScriptLanguage lang) =>
         (MonadReader (TestSuiteRunnerInternalEnv lang era) m) =>
           TestFaucetBudget -> FaucetsNumber -> m (TVar [TestRunnerFaucet era])
    ...
    do let faucetNicknames =
       ...
       execWalletOperations interpretState operations >>= \case
         Left (err, interpretState') -> do
           ...
           liftIO $
             throwIO $
             TestRunnerError $ "Failed to create faucet wallets: " <> show err
...
testSuiteResultToJSON ::
     (C.IsCardanoEra era)
  => (MonadIO m) =>
       TestSuiteResult lang era -> m A.Value
...
runTests ::
     forall era lang. (IsShelleyBasedEra era)
  => (IsPlutusScriptLanguage lang) =>
       [(FilePath, TestCase)] -> ConcurrentRunners -> TestSuiteRunnerM lang era (TestSuiteResult lang era)
runTests tests (ConcurrentRunners maxConcurrentRunners) = do
  ...
  liftIO
    do hPutStrLn stderr $
         ...
         show (toInteger requiredFunds `div` 1 _000_000) <> " ADA"
       hPutStrLn stderr $
         "Estimated subfaucet budget: " <>
         show (toInteger subfaucetBudget `div` 1 _000_000) <> " ADA"
       hPutStrLn stderr $
         "Estimated test total tx fees: " <>
         show (toInteger totalTxCost `div` 1 _000_000) <> " ADA"
       ...
       for_ tests $ \(testFile, operations) -> do
         ...
         hPutStrLn stderr $
           ...
           show (toInteger budgetUpperBound `div` 1 _000_000) <> " ADA"
         hPutStrLn stderr $
           "Estimated test tx fees: " <>
           show (toInteger txsFeesUpperBound `div` 1 _000_000) <> " ADA"
