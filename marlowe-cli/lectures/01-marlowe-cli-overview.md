...
## Three Levels of Interaction with Marlowe on the Blockchain
  ...
  ------------------- --------------------- ------------------------------
  • Supports          • Supports a          • Supports direct interaction
  developer workflows straightforward       with backend contracts and
  for debugging and   workflow for users    endpoints for running Marlowe
  fine-grained        that just want to run from the command line.
  control of each     contracts from the    
  atomic operation    command line.         
  involved in running                       
  Marlowe contracts.                        

  • Controls          • Hides details of    • Hides details of UTxO
  modification of     input and state of    management by interacting with
  Marlowe state and   Marlowe contracts.    a Cardano Wallet or Daedalus.
  construction of                           
  Marlowe input.                            

  • Controls          • Hides and automates • Mimics the workflow for
  construction and    many aspects of       using Marlowe Run.
  submission of       Plutus and            
  validators, datums, interaction with the  
  and redeemers.      Cardano node.         

  • Focus on the      • Focuses on the      • Focuses on user interaction.
  mechanics of        Marlowe contract.     
  Marlowe on Plutus                         
  and Cardano.                              
  ------------------------------------------------------------------------
...
## Marlowe on the Blockchain
  ...
  -------------- --------------------------------- ------------ ---------------
  Minting role   One-shot monetary policy          n/a          n/a
  tokens                                                        

  Marlowe        Generic for all Marlowe           Unexecuted   Input to the
  application    contracts, but whose address is   parts of the next step of
                 specialized to the roles for the  contract;    the contract,
                 contract                          balances for such as
                                                   accounts;    deposits,
                                                   records of   choices, or
                                                   past inputs; notifications
                                                   values of    
                                                   parameters   

  Marlowe        Specific to particular role       Role that is n/a
  payouts        currency                          to be paid.  
  -----------------------------------------------------------------------------
...
## High-Level Marlowe CLI Commands
  ...
  `run initialize`              Initialize the first transaction of a
                                Marlowe contract and write output to a
                                JSON file.

  `run prepare`                 Prepare the next step of a Marlowe
                                contract and write the output to a JSON
                                file.

  `run withdraw`                Withdraw funds from the Marlowe role
                                address.
...
## Low-Level Marlowe CLI Commands (2 of 2)
  ...
  Command                       Description
  ----------------------------- -----------------------------------------
  `role`                        Export role address, validator, datum, or
                                redeemer.
  ...
  `transaction advance`         Build a transaction that both spends from
                                and pays to a Marlowe script.

  `transaction close`           Build a transaction that spends from a
                                Marlowe script.

  `transaction create`          Build a transaction that pays to a
                                Marlowe script.
