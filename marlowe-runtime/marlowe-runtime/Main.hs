...
getOptions = do
  ...
  where
    parser defaultNetworkId defaultSocketPath ContractStoreOptions{..} = Options
      <$> databaseUriParser
      <*> hostParser
      <*> portParser
      <*> portTracedParser
    ...
    blockCostParser = option auto $ mconcat
      [ long "block-cost"
      , value 1
      , metavar "COST_UNITS"
    ...
    txCostParser = option auto $ mconcat
      [ long "tx-cost"
      , value 10
      , metavar "COST_UNITS"
    ...
    maxCostParser = option auto $ mconcat
      [ long "max-cost"
      , value 100_000
      , metavar "COST_UNITS"
      , help "The maximum number of cost units that can be batched when persisting blocks. If the cost of the current batch would exceed this value, the chain sync client will wait until the current batch is persisted before requesting another block."
      , showDefault
      ]
