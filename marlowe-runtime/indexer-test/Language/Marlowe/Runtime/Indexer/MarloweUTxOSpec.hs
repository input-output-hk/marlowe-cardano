...
extractCreateTxSpec = describe "extractCreateTx" do
  ...
  prop "if it extracts nothing, it doesn't change the UTxO" \tx utxo ->
    let (txs, utxo') = runState (execWriterT $ extractCreateTx marloweScriptHashes tx) utxo
     in null txs ==> utxo' === utxo
    ...
    forAll (createTxToChainTx Nothing createTx) \tx ->
      let (txs, utxo') = runState (execWriterT $ extractCreateTx marloweScriptHashes tx) utxo
          addedContracts = flip foldMap txs \case
            ...
            _ -> mempty
       in Map.difference (unspentContractOutputs utxo') (unspentContractOutputs utxo) === addedContracts
  prop "Emits invalid create transactions" $ forAll (genCreateTx marloweScripts) \createTx utxo ->
    forAll genCreateBug \bug ->
      forAll (createTxToChainTx (Just bug) createTx) \tx ->
        let txs = evalState (execWriterT $ extractCreateTx marloweScriptHashes tx) utxo
...
extractApplyInputsTxSpec = describe "extractApplyInputsTx" do
  ...
  prop "if it extracts nothing, it doesn't change the UTxO" \tx utxo ->
    let (txs, utxo') = runState (execWriterT $ extractApplyInputsTx' testBlockHeader tx) utxo
     in null txs ==> utxo' === utxo
  prop "Preserves contracts in the UTxO if there is a script output" $ forAll (genApplyTx testBlockHeader) \(inputDatum, applyTx) ->
    forAll (applyTxToChainTx inputDatum Nothing applyTx) \tx ->
      forAll (addInput applyTx <$> arbitrary) \utxo ->
        let (result, utxo') = runState (execWriterT $ extractApplyInputsTx' testBlockHeader tx) utxo
            applyTx' =
              asum $
                result <&> \case
            ...
            hasScriptOut = case applyTx' of
              ...
              Nothing -> False
         in hasScriptOut ==> Map.keysSet (unspentContractOutputs utxo') === Map.keysSet (unspentContractOutputs utxo)
        ...
        let (result, utxo') = runState (execWriterT $ extractApplyInputsTx' testBlockHeader tx) utxo
            ...
            expectedOut = case applyTx' of
              ...
              Just (MarloweApplyInputsTransaction Core.MarloweV1 UnspentContractOutput{..} marloweTransaction) ->
                Core.scriptOutput (Core.output marloweTransaction) <&> \Core.TransactionScriptOutput{utxo = scriptOut} ->
                  UnspentContractOutput
                    { marloweVersion
                    , txOutRef = scriptOut
                    , marloweAddress
         ...
         in Map.lookup contractId (unspentContractOutputs utxo') === expectedOut
  prop "Only adds payouts" $ forAll (genApplyTx testBlockHeader) \(inputDatum, applyTx) ->
    forAll (applyTxToChainTx inputDatum Nothing applyTx) \tx ->
      forAll (addInput applyTx <$> arbitrary) \utxo ->
        let utxo' = execState (execWriterT $ extractApplyInputsTx' testBlockHeader tx) utxo
            diffAndRemoveEmpty a b = mfilter (not . Set.null) $ Just $ Set.difference a b
         in Map.differenceWith diffAndRemoveEmpty (unspentPayoutOutputs utxo) (unspentPayoutOutputs utxo') === mempty
  ...
  prop "Doesn't leave empty payout sets" $ forAll (genApplyTx testBlockHeader) \(inputDatum, applyTx) ->
    forAll (applyTxToChainTx inputDatum Nothing applyTx) \tx ->
      forAll (addInput applyTx <$> arbitrary) \utxo ->
        let utxo' = execState (execWriterT $ extractApplyInputsTx' testBlockHeader tx) utxo
         in Map.filter Set.null (unspentPayoutOutputs utxo') === mempty
        ...
        let (result, utxo') = runState (execWriterT $ extractApplyInputsTx' testBlockHeader tx) utxo
            ...
            payouts =
              result & foldMap \case
                ...
                _ -> mempty
         in Set.difference newValue oldValue === payouts

addInput :: MarloweApplyInputsTransaction -> MarloweUTxO -> MarloweUTxO
addInput MarloweApplyInputsTransaction{..} utxo =
  utxo
...
extractWithdrawTxSpec = describe "extractWithdrawTx" do
  prop "writes nothing if no payouts were withdrawn" \txId txIns utxo ->
    all (Set.null . Set.intersection (Set.map inputToTxOutRef txIns)) (unspentPayoutOutputs utxo) ==>
      let tx = Chain.Transaction txId Chain.Unbounded mempty txIns [] mempty
       in evalState (execWriterT $ extractWithdrawTx tx) utxo === []
  prop "does not update the marlowe UTxO if it produces nothing" \txId txIns utxo ->
    let tx = Chain.Transaction txId Chain.Unbounded mempty txIns [] mempty
     ...
     in null result ==> utxo === utxo'
  prop "Tx contains all spent payouts" \txId utxo ->
    not (Map.null $ unspentPayoutOutputs utxo) ==>
      forAll
        ( Set.fromList
            <$> (zipWith mkTxIn <$> infiniteListOf arbitrary <*> sublistOf (Map.elems (unspentPayoutOutputs utxo) >>= Set.toList))
        )
        \txIns ->
          let tx = Chain.Transaction txId Chain.Unbounded mempty txIns [] mempty
  ...
  prop "Payouts are conserved" \txId txIns utxo ->
    let tx = Chain.Transaction txId Chain.Unbounded mempty txIns [] mempty
        ...
        consumed = flip foldMap txs \case
          ...
          _ -> mempty
     in Map.unionWith (<>) consumed (unspentPayoutOutputs utxo') == unspentPayoutOutputs utxo
  prop "Payouts aren't duplicated" \txId txIns utxo ->
    let tx = Chain.Transaction txId Chain.Unbounded mempty txIns [] mempty
        ...
        consumed = flip foldMap txs \case
          WithdrawTransaction MarloweWithdrawTransaction{..} -> consumedPayouts
          _ -> mempty
     in Map.filter (not . Set.null) (Map.intersectionWith Set.intersection consumed (unspentPayoutOutputs utxo')) == mempty
...
applyTxToChainTx inputDatum _ MarloweApplyInputsTransaction{..} = case marloweVersion of
  Core.MarloweV1 -> do
    let Core.Transaction{..} = marloweTransaction
    ...
    let scriptChainOutputs =
          maybeToList $
            scriptOutput <&> \Core.TransactionScriptOutput{..} ->
              Chain.TransactionOutput
                { address
                , assets
        ...
        payoutOutputs =
          Map.toAscList payouts <&> \(_, Core.Payout{..}) ->
            let chainDatum = Core.toChainPayoutDatum Core.MarloweV1 datum
...
genApplyTx blockHeader = do
  ...
  let marloweParams = V1.MarloweParams $ toPlutusCurrencySymbol rolesCurrency
      genInputs = do
        context <- arbitrary
        marloweState <- semiArbitrary context
        marloweContract <- semiArbitrary context
        let boundTimes V1.TransactionInput{..} =
              V1.TransactionInput
  ...
  isClose <- arbitrary
  let (minPOSIXTime, maxPOSIXTime) = txInterval
...
instance Arbitrary UnspentContractOutput where
  arbitrary =
    UnspentContractOutput (Core.SomeMarloweVersion Core.MarloweV1)
      <$> arbitrary
      <*> arbitrary
