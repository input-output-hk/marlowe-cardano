...
yaml2json << EOI > choose-1.contract
...
- case:
  ...
  then: close
EOI
cat choose-1.contract

CONTRACT_ID=$(
marlowe create \
  --core-file choose-1.contract \
  --min-utxo "$MINIMUM_ADA" \
  --change-address "$PARTY_ADDR" \
  --manual-sign choose-1.txbody \
| sed -e 's/^.*"\([^\\]*\)\\.*$/\1/' \
)
echo "CONTRACT_ID = $CONTRACT_ID"

cardano-cli transaction sign \
  --tx-body-file choose-1.txbody \
  --out-file     choose-1.tx \
  --signing-key-file "$PARTY_SKEY"

marlowe submit choose-1.tx
...
marlowe choose \
  ...
  --change-address "$PARTY_ADDR" \
  --manual-sign choose-2.txbody

cardano-cli transaction sign \
  --tx-body-file choose-2.txbody \
  --out-file     choose-2.tx \
  --signing-key-file "$PARTY_SKEY"

marlowe submit choose-2.tx
