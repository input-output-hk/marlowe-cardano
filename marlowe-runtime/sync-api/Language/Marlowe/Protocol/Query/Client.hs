...
getWithdrawals
  :: (Applicative m) => WithdrawalFilter -> Range TxId -> MarloweQueryClient m (Maybe (Page TxId Withdrawal))
getWithdrawals = fmap request . ReqWithdrawals

getContractHeaders
  :: (Applicative m) => ContractFilter -> Range ContractId -> MarloweQueryClient m (Maybe (Page ContractId ContractHeader))
