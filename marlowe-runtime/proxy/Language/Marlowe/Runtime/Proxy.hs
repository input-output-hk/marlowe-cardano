...
server useOpenRefAsParent Router{..} =
  MarloweRuntimeServer
...
marloweHeaderSyncNext
  :: PeerHasAgency pr st -> Message MarloweHeaderSync st st' -> NextAgency MarloweHeaderSync pr st st'
...
withHandshake useOpenRefAsParent agency openConnection nextAgency = do
  ...
  let runMain
        | useOpenRefAsParent =
            fmap (hoistPeerTraced (localBackend (setAncestorEventBackend openRef))) . localBackend (setAncestorEventBackend openRef)
        | otherwise = id
  runMain do
