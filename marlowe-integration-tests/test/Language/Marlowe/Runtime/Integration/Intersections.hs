...
spec = it "Intersections" $ withLocalMarloweRuntime $ runIntegrationTest do
  ...
  let ContractCreated _ ContractCreatedInEra{contractId = idA} = contractCreated contractA
...
completeContract StandardContractLifecycleInit{..} = do
  ...
  pure case initialFundsDeposited of
    InputsApplied _ InputsAppliedInEra{txBody} ->
      ( initialDepositBlock
