...
setup runSpec = withLocalMarloweRuntime $ runIntegrationTest do
  ...
  liftIO . runSpec . snd =<< submitBuilder (genesisWallet0 <> genesisWallet1) do
    ...
    singleAddressSufficientBalanceOneInsufficientCollateralWallet <-
      fmap addresses $ allocateWallet [[(True, 857690), (False, 12_000_000)]]
    singleAddressSufficientBalanceMultiInsufficientCollateralWallet <-
      fmap addresses $ allocateWallet [[(True, 857690), (True, 857690), (False, 12_000_000)]]
    ...
    singleAddressSufficientBalanceMultiSufficientCollateralWallet <-
      fmap addresses $ allocateWallet [[(True, 1_500_000), (True, 2_000_000), (False, 10_000_000)]]
    ...
    multiAddressSufficientBalanceOneInsufficientCollateralWallet <-
      fmap addresses $ allocateWallet [[(True, 857690), (False, 1_000_000)], [(False, 1_500_000), (False, 10_000_000)]]
    multiAddressSufficientBalanceMultiInsufficientCollateralWallet <-
      fmap addresses $ allocateWallet [[(True, 857690), (True, 857690)], [(False, 11_000_000)]]
    multiAddressSufficientBalanceOneSufficientCollateralWallet <-
      fmap addresses $ allocateWallet [[(False, 5_000_000)], [(True, 2_000_000), (False, 10_000_000)]]
    multiAddressSufficientBalanceMultiSufficientCollateralWallet <-
      fmap addresses $ allocateWallet [[(True, 1_000_000), (True, 2_000_000)], [], [(False, 10_000_000)]]
    pure
      TestData
...
expectedMetadata (MetadataCase marloweMetadata extraMetadata) = MarloweTransactionMetadata
  ...
  case marloweMetadata of
    ...
    EmptyTags ->
      TransactionMetadata $
        Map.insert 1564 (encodeMarloweMetadata $ MarloweMetadata mempty Nothing) $
          unTransactionMetadata $
            mkExtraMetadata extraMetadata
    NonEmptyTags ->
      TransactionMetadata $
        Map.insert 1564 (encodeMarloweMetadata testMarloweMetadata) $
          unTransactionMetadata $
            mkExtraMetadata extraMetadata
...
mkMarloweTxMetadata (MetadataCase marloweMetadata extraMetadata) =
  MarloweTransactionMetadata
