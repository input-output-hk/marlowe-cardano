...
let
  ...
  symlinks = runCommand "symlinks" { } ''
    ...
    ln -sv ${run-local-service "marlowe-chain-sync" marloweChainSyncVersion "marlowe-chain-indexer"}/bin/run-marlowe-chain-indexer $out
    ln -sv ${run-local-service "marlowe-chain-sync" marloweChainSyncVersion "marlowe-chain-sync"}/bin/run-marlowe-chain-sync $out
    ...
    ln -sv ${run-local-service "marlowe-runtime-web" "0.0.1" "marlowe-web-server"}/bin/run-marlowe-web-server $out
