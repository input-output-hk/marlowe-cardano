...
in
{
  ...
  marlowe-contract = mkOperableWithProbes {
    ...
    runtimeScript = ''
      ...
      # REQUIRED VARS #
      #################
      # HOST, PORT, QUERY_PORT, IMPORT_PORT: network binding
      ...
      [ -z "''${PORT:-}" ] && echo "PORT env var must be set -- aborting" && exit 1
      [ -z "''${QUERY_PORT:-}" ] && echo "QUERY_PORT env var must be set -- aborting" && exit 1
      [ -z "''${IMPORT_PORT:-}" ] && echo "IMPORT_PORT env var must be set -- aborting" && exit 1
  ...
  marlowe-proxy = mkOperableWithProbes {
    ...
    runtimeScript = ''
      ...
      # CONTRACT_HOST, LOAD_PORT, CONTRACT_QUERY_PORT, IMPORT_PORT: connection info to marlowe-contract
      # HTTP_PORT: port number for the HTTP healthcheck server
      ...
      ${marlowe-proxy}/bin/marlowe-proxy \
        ...
        --marlowe-import "$LOAD_PORT" \
        --marlowe-import-port "$IMPORT_PORT" \
        --contract-query-port "$CONTRACT_QUERY_PORT" \
        --tx-host "$TX_HOST" \
