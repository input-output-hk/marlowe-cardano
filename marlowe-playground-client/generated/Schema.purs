...
instance encodeJsonFormArgumentF ::
  ...
  encodeJson = defer \_ -> case _ of
    ...
    FormRadioF a b -> E.encodeTagged "FormRadioF" (a /\ b)
      (E.tuple (E.value >/\< (E.maybe E.value)))
    FormArrayF a b -> E.encodeTagged "FormArrayF" (a /\ b)
      (E.tuple (E.value >/\< E.value))
    FormMaybeF a b -> E.encodeTagged "FormMaybeF" (a /\ b)
      (E.tuple (E.value >/\< (E.maybe E.value)))
    FormTupleF a b -> E.encodeTagged "FormTupleF" (a /\ b)
      (E.tuple (E.value >/\< E.value))
...
instance decodeJsonFormArgumentF ::
  ...
  decodeJson = defer \_ -> D.decode
    ...
    $ Map.fromFoldable
        ...
        , "FormRadioF" /\ D.content
            (D.tuple $ FormRadioF </$\> D.value </*\> (D.maybe D.value))
        , "FormArrayF" /\ D.content
            (D.tuple $ FormArrayF </$\> D.value </*\> D.value)
        , "FormMaybeF" /\ D.content
            (D.tuple $ FormMaybeF </$\> D.value </*\> (D.maybe D.value))
        , "FormTupleF" /\ D.content
            (D.tuple $ FormTupleF </$\> D.value </*\> D.value)
...
_FormRadioF
  :: forall a. Prism' (FormArgumentF a) { a :: Array String, b :: Maybe String }
_FormRadioF = prism' (\{ a, b } -> (FormRadioF a b)) case _ of
  (FormRadioF a b) -> Just { a, b }
  _ -> Nothing

_FormArrayF
  :: forall a. Prism' (FormArgumentF a) { a :: FormSchema, b :: Array a }
_FormArrayF = prism' (\{ a, b } -> (FormArrayF a b)) case _ of
  (FormArrayF a b) -> Just { a, b }
  _ -> Nothing

_FormMaybeF
  :: forall a. Prism' (FormArgumentF a) { a :: FormSchema, b :: Maybe a }
