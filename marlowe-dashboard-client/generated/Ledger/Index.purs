...
instance encodeJsonValidationError :: EncodeJson ValidationError where
  encodeJson = defer \_ -> case _ of
    ...
    InvalidScriptHash a b -> E.encodeTagged "InvalidScriptHash" (a /\ b)
      (E.tuple (E.value >/\< E.value))
    InvalidDatumHash a b -> E.encodeTagged "InvalidDatumHash" (a /\ b)
      (E.tuple (E.value >/\< E.value))
    MissingRedeemer a -> E.encodeTagged "MissingRedeemer" a E.value
    InvalidSignature a b -> E.encodeTagged "InvalidSignature" (a /\ b)
      (E.tuple (E.value >/\< E.value))
...
instance decodeJsonValidationError :: DecodeJson ValidationError where
  decodeJson = defer \_ -> D.decode
    ...
    $ Map.fromFoldable
        [ "InOutTypeMismatch" /\ D.content
            (D.tuple $ InOutTypeMismatch </$\> D.value </*\> D.value)
        , "TxOutRefNotFound" /\ D.content (TxOutRefNotFound <$> D.value)
        , "InvalidScriptHash" /\ D.content
            (D.tuple $ InvalidScriptHash </$\> D.value </*\> D.value)
        , "InvalidDatumHash" /\ D.content
            (D.tuple $ InvalidDatumHash </$\> D.value </*\> D.value)
        , "MissingRedeemer" /\ D.content (MissingRedeemer <$> D.value)
        , "InvalidSignature" /\ D.content
            (D.tuple $ InvalidSignature </$\> D.value </*\> D.value)
...
_TransactionFeeTooLow = prism' (\{ a, b } -> (TransactionFeeTooLow a b))
  case _ of
