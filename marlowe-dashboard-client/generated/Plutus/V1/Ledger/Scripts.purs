...
instance decodeJsonMintingPolicy :: DecodeJson MintingPolicy where
  decodeJson = defer \_ -> D.decode $
    ( MintingPolicy <$> D.record "MintingPolicy"
        { getMintingPolicy: D.value :: _ String }
    )
...
instance decodeJsonStakeValidator :: DecodeJson StakeValidator where
  decodeJson = defer \_ -> D.decode $
    ( StakeValidator <$> D.record "StakeValidator"
        { getStakeValidator: D.value :: _ String }
    )
...
instance decodeJsonValidator :: DecodeJson Validator where
  decodeJson = defer \_ -> D.decode $
    (Validator <$> D.record "Validator" { getValidator: D.value :: _ String })
...
instance encodeJsonScriptError :: EncodeJson ScriptError where
  encodeJson = defer \_ -> case _ of
    EvaluationError a b -> E.encodeTagged "EvaluationError" (a /\ b)
      (E.tuple (E.value >/\< E.value))
    EvaluationException a b -> E.encodeTagged "EvaluationException" (a /\ b)
      (E.tuple (E.value >/\< E.value))
...
instance decodeJsonScriptError :: DecodeJson ScriptError where
  decodeJson = defer \_ -> D.decode
    ...
    $ Map.fromFoldable
        [ "EvaluationError" /\ D.content
            (D.tuple $ EvaluationError </$\> D.value </*\> D.value)
        , "EvaluationException" /\ D.content
            (D.tuple $ EvaluationException </$\> D.value </*\> D.value)
